                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden p-6 space-y-4">
                    ...
                    <div class="flex items-center justify-between border-b border-gray-50 pb-3">
                        ...
                        <div class="flex items-center gap-1">
                            <a href="{{ route('admin.categories.edit', $category->id) }}" class="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg transition-all" title="Tahrirlash">
                                <i class="fa-solid fa-pen-to-square"></i>
                            </a>
                            <form action="{{ route('admin.categories.delete', $category->id) }}" method="POST" onsubmit="return confirm('Haqiqatdan ham ushbu kategoriyani va uning barcha sub-kategoriyalarini o\'chirmoqchimisiz?');" class="inline">
                                @csrf
                                @method('DELETE')
                                ...
                                @foreach($category->subCategories as $sub)
                                    <div class="flex items-center gap-2 px-3 py-1.5 bg-gray-50 hover:bg-gray-100 rounded-xl border border-gray-100 text-xs font-semibold text-gray-600 transition-all">
                                        ...
                                        <div class="flex items-center gap-1 ml-2 border-l border-gray-200 pl-2">
                                            <a href="{{ route('admin.subcategories.edit', $sub->id) }}" class="text-blue-500 hover:text-blue-700" title="Tahrirlash">
                                                <i class="fa-solid fa-pen text-[10px]"></i>
                                            </a>
                                            <form action="{{ route('admin.subcategories.delete', $sub->id) }}" method="POST" onsubmit="return confirm('Haqiqatdan ham ushbu sub-kategoriyani o\'chirmoqchimisiz?');" class="inline">
                                                @csrf
                                                @method('DELETE')
