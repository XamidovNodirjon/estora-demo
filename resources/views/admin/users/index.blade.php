...
@section('header_title', 'Foydalanuvchilar Ro\'yxati')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
        <div>
            ...
            <p class="text-xs text-gray-400 mt-1">Ro'yxatdan o'tgan mijozlar va tizim xodimlari</p>
        </div>
        <a href="{{ route('admin.users.create') }}" class="px-5 py-2.5 bg-[#0084ff] hover:bg-[#0076e5] text-white text-sm font-semibold rounded-xl flex items-center gap-2 shadow-lg shadow-blue-500/20 transition-all">
            <i class="fa-solid fa-user-plus"></i>
            <span>Yangi xodim / mijoz</span>
        </a>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div class="flex border-b border-gray-100 bg-gray-50/50 px-6 pt-2">
            <a href="{{ route('admin.users', ['tab' => 'staff']) }}" 
               class="px-5 py-3 border-b-2 font-semibold text-sm transition-all flex items-center gap-2 {{ $tab === 'staff' ? 'border-[#0084ff] text-[#0084ff]' : 'border-transparent text-gray-400 hover:text-gray-600' }}">
                <i class="fa-solid fa-user-tie"></i>
                <span>Tizim xodimlari</span>
            </a>
            <a href="{{ route('admin.users', ['tab' => 'clients']) }}" 
               class="px-5 py-3 border-b-2 font-semibold text-sm transition-all flex items-center gap-2 {{ $tab === 'clients' ? 'border-[#0084ff] text-[#0084ff]' : 'border-transparent text-gray-400 hover:text-gray-600' }}">
            ...
            <table class="w-full text-left border-collapse text-sm">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-200 text-gray-500 font-semibold">
                        ...
                        <th class="px-6 py-4">Yaratilgan sana</th>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-8 w-full max-w-2xl mx-4 relative z-10 shadow-2xl transition-all duration-300 transform scale-95 opacity-0" id="editModalContent">
        ...
        <form id="editUserForm" action="" method="POST" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    <label for="edit_username" class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Username (Ixtiyoriy)</label>
...
<script>
    ...
    function openEditModal(btn) {
        ...
        document.getElementById('edit_username').value = btn.dataset.username !== 'null' ? (btn.dataset.username || '') : '';
        document.getElementById('edit_email').value = btn.dataset.email;
        document.getElementById('edit_phone').value = btn.dataset.phone !== 'null' ? (btn.dataset.phone || '') : '';
        document.getElementById('edit_role_id').value = btn.dataset.roleId;
        document.getElementById('edit_type').value = btn.dataset.type;
        document.getElementById('edit_passport').value = btn.dataset.passport !== 'null' ? (btn.dataset.passport || '') : '';
        document.getElementById('edit_jshshir').value = btn.dataset.jshshir !== 'null' ? (btn.dataset.jshshir || '') : '';
