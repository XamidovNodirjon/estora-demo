...
<div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm mb-6 flex justify-between items-center">
    ...
    <a href="{{ route('developer.metros.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700">Yangi qo'shish</a>
</div>
<div class="space-y-4">
    @forelse($metros as $item)
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex justify-between items-center">
            ...
            <div class="flex gap-2">
                <a href="{{ route('developer.metros.edit', $item->id) }}" class="text-blue-600 hover:bg-blue-50 p-2 rounded">Tahrirlash</a>
                <form action="{{ route('developer.metros.delete', $item->id) }}" method="POST" onsubmit="return confirm('O\'chirishni xohlaysizmi?');">
                    @csrf @method('DELETE')
