...
@section('header_title', 'Foydalanuvchi Tahrirlash')

@section('content')
<div class="max-w-3xl mx-auto space-y-6">
    <!-- Header Card -->
    <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4">
        <a href="{{ route('developer.users') }}" class="w-10 h-10 rounded-xl bg-gray-50 border border-gray-100 flex items-center justify-center text-gray-500 hover:bg-gray-100 transition-all">
            <i class="fa-solid fa-arrow-left"></i>
        </a>
        <div>
            <h2 class="font-display font-bold text-lg text-[#061c3f]">Foydalanuvchi ma'lumotlarini tahrirlash</h2>
            <p class="text-xs text-gray-400">Tizimdagi foydalanuvchi profil ma'lumotlarini yangilash formasi</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-8 shadow-sm">
        ...
        <form action="{{ route('developer.users.update', $user->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    <label for="username" class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Foydalanuvchi nomi (Username)</label>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="type" id="type" required
                        ...
                        <option value="dev" {{ old('type', $user->type) == 'dev' ? 'selected' : '' }}>Developer</option>
                        <option value="admin" {{ old('type', $user->type) == 'admin' ? 'selected' : '' }}>Admin (Admin/Staff)</option>
                        <option value="manager" {{ old('type', $user->type) == 'manager' ? 'selected' : '' }}>Manager (Admin/Staff)</option>
                        <option value="client" {{ old('type', $user->type) == 'client' ? 'selected' : '' }}>Client (Mijoz)</option>
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="passport" id="passport" value="{{ old('passport', $user->passport) }}"
                        class="block w-full px-4 py-2.5 rounded-xl bg-gray-50 border border-gray-200 text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#0084ff] focus:bg-white transition-all text-sm"
                ...
                <div>
                    ...
                    <input type="text" name="jshshir" id="jshshir" value="{{ old('jshshir', $user->jshshir) }}"
                        class="block w-full px-4 py-2.5 rounded-xl bg-gray-50 border border-gray-200 text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#0084ff] focus:bg-white transition-all text-sm"
            ...
            <!-- Password -->
            <div>
                <label for="password" class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Yangi parol (Ixtiyoriy)</label>
            ...
            <div class="flex items-center justify-end gap-4 border-t border-gray-100 pt-6">
                <a href="{{ route('developer.users') }}" class="px-5 py-2.5 bg-gray-50 hover:bg-gray-100 text-gray-600 font-semibold text-sm rounded-xl transition-all">
                    Bekor qilish
                </a>
